...
def load_config() -> dict:
    with open('etc/mailing/config.yaml', 'r', encoding="utf-8") as f:
        return yaml.load(f, Loader=yaml.FullLoader)
...
def build_email_content(
    ...
    <body class="container">
        <header style="text-align: center;">
            <img src=https://storage.googleapis.com/{global_config["stallion_photos_bucket_name"]}/{config["logo_filename"]} alt="Logo" width="350">
        </header>
...
def send_action_email(
    ...
    message["To"] = send_to
    message["From"] = f'{global_config["company_name"]} <{config["declared_sender_email"]}>'
