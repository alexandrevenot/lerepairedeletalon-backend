...
from context import fake_db, get_db, SMTPDummySession, get_db_client
...
class paymentsTest(unittest.TestCase):
    def test(self):
        ...
        with patch('app.payments.router.stripe', new_callable=Mock), \
        patch('app.payments.router.stripe.Account.create', new_callable=Mock) as mock1, \
        patch('app.payments.router.stripe.Account.create_person', new_callable=Mock) as mock2, \
        patch('app.payments.router.stripe.Account.create_external_account', new_callable=Mock) as mock3, \
        patch('app.payments.router.stripe.Webhook.construct_event', new_callable=Mock) as mock4:
