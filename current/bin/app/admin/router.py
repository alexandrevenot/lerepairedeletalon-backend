...
@router.get('/users-whose-bank-identity-files-is-to-be-validated', response_model=schemas.IdList)
async def get_users_whose_bank_identity_files_is_to_be_validated(_ = Depends(verify_admin_password), db = Depends(get_db)):
    id_list = []
    try:
        for document in db.users.find({"bank_identity.bank_identity_file_status": "to_be_validated"}, {"_id": 1}):
            id_list.append(str(document["_id"]))
    except PyMongoError as exc:
        logger.error("failed to read db: %s", traceback.format_exc())
        raise HTTPException(status_code=500, detail="failed to read db") from exc
...
@router.put('/bank-identity/{user_id}')
async def put_bank_identity(_ = Depends(verify_admin_password), user_in_db = Depends(get_user_in_db)):
    pass
